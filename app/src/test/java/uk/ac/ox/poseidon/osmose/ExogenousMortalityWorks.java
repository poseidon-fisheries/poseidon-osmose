...
public class ExogenousMortalityWorks {
    ...
    public void murderResponsibly() throws Exception {
        ...
        final URL restartFile = getClass().getClassLoader().getResource("test.nc");
        assert restartFile != null;

        final URL configuration = getClass().getClassLoader().getResource("osmose_config/osm_all-parameters.csv");
        assert configuration != null;
        OsmoseSimulation simulation =
                OsmoseSimulation.startupOSMOSEWithRestartFile(0, configuration.getPath(), restartFile.getPath());

        ExogenousMortality mortality = simulation.getMortality();
        for (int i = 0; i < 10; i++) mortality.markThisSpeciesAsExogenous(i);
        ...
        for (Map.Entry<School, Double> catches : fishCaught.entrySet()) {
            ...
            assertEquals(catches.getValue(), catches.getKey().getNdead(MortalityCause.FISHING), .1);
        }
    ...
    public void osmoseMurderSome() throws Exception {
        ...
        ExogenousMortality mortality = simulation.getMortality();
        for (int i = 0; i < 5; i++) mortality.markThisSpeciesAsExogenous(i);
        ...
        for (School school : simulation.getSchoolSet()) {
            ...
            if (school.getSpeciesIndex() < 5) assertEquals(0, deads, .1);
            if (school.getSpeciesIndex() >= 5 && deads > 0) osmoseKilled++;
        }
