...
public class OsmoseWFSScenario implements Scenario {
    ...
    public static final Path mainDirectory = Paths.get("temp_wfs");
    private final OsmoseBoundedMapInitializerFactory mapInitializer = new OsmoseBoundedMapInitializerFactory();
    private final AlgorithmFactory<? extends WeatherInitializer> weatherInitializer = new TimeSeriesWeatherFactory(
        Paths.get("temp_wfs", "steve", "Fleet", "PriceWSgas.txt").toString(), true, '$', 10);
    private final AlgorithmFactory<? extends HabitatInitializer> habitatInitializer = new AllSandyHabitatFactory();
    //
    private final NetworkBuilder networkBuilder = new EquidegreeBuilder();
    ...
    private AlgorithmFactory<? extends LogbookInitializer> longlineLogbook = new LogisticLogbookFactory();
    private AlgorithmFactory<? extends LogbookInitializer> handlineLogbook = new NoLogbookFactory();
    ...
    private AlgorithmFactory<? extends DiscardingStrategy> handlinerDiscardingStrategy =
        new DiscardingAllUnsellableFactory();
    /**
     ...
     */
    private AlgorithmFactory<? extends FishingStrategy> handlinerFishingStrategy = new FloridaLogitReturnFactory();
    private AlgorithmFactory<? extends GearStrategy> handlinerGearStrategy = new FixedGearStrategyFactory();
    private AlgorithmFactory<? extends WeatherEmergencyStrategy> handlinerWeatherStrategy = new IgnoreWeatherFactory();
     ...
     */
    // this number comes from SEDAR estimate of 1.5M fish aged 0-1 killed by shrimp bycatch weighted at 83.1g which is
    // the 6 months weight of the fish
    ...
    private AlgorithmFactory<? extends DiscardingStrategy> longlinerDiscardingStrategy =
        new DiscardingAllUnsellableFactory();
    ...
    // (3)GAG GROUPER:
    // SWG quota (for gag grouper) was 3991612.856 (8.8 million pounds, but this was the whole SWG; SEDAR 33 table
    // 2.6.1)
     ...
     * factory to produce fishing strategy
     */
    private AlgorithmFactory<? extends FishingStrategy> longlinerFishingStrategy = new FloridaLogitReturnFactory();
    // RED GROUPER:
    // 2,843,775 kg (2003 to 2006)
    // 2,024,700 kg (2007 to 2010)
    private AlgorithmFactory<? extends GearStrategy> longlinerGearStrategy = new FixedGearStrategyFactory();
    private AlgorithmFactory<? extends WeatherEmergencyStrategy> longlinerWeatherStrategy = new IgnoreWeatherFactory();
    ...
    private DoubleParameter longlinerSpeedKph =
        new FixedDoubleParameter(16.0661); // this is 8.675 knots from the data request
    private DoubleParameter handlinerSpeedKph =
        new FixedDoubleParameter(16.0661); // this is 8.675 knots from the data request

    {
        biologyInitializer.setIndexOfSpeciesToBeManagedByThisModel("2,3,4");
        biologyInitializer.setOsmoseConfigurationFile(mainDirectory
            .resolve("wfs")
            .resolve("osm_all-parameters.csv")
            .toAbsolutePath()
            .toString());
        biologyInitializer.setPreInitializedConfigurationDirectory(mainDirectory
            .resolve("wfs")
            .resolve("randomStarts")
            .toAbsolutePath()
            .toString());
        ...
        biologyInitializer
            .getDiscardMortalityRate()
            .put(2, .374); // red grouper SEDAR 42, table 2.12 (assuming the most common depth of fishing)
        biologyInitializer
            .getDiscardMortalityRate()
            .put(3, .5); // gag grouper SEDAR 33, figure 6.1 (although it really is a function)
        biologyInitializer
            .getDiscardMortalityRate()
            .put(4, 0.82875); // red snapper SEDAR 31, table 5.1, average no-venting mortality
    }
    ...
    {
        ((FloridaLogisticDepartingFactory) handlinerDepartingStrategy)
            .setIntercept(new FixedDoubleParameter(-2.075184));
        ...
        ((FloridaLogisticDepartingFactory) handlinerDepartingStrategy)
            .setWindSpeedInKnots(new FixedDoubleParameter(-0.046672));
        ((FloridaLogisticDepartingFactory) handlinerDepartingStrategy)
            .setRealDieselPrice(new FixedDoubleParameter(-0.515073 / 0.219969157));
        ((FloridaLogisticDepartingFactory) handlinerDepartingStrategy)
            .setPriceRedGrouper(new FixedDoubleParameter(-0.3604 / 2.20462262));
        ((FloridaLogisticDepartingFactory) handlinerDepartingStrategy)
            .setPriceGagGrouper(new FixedDoubleParameter(0.649616 / 2.20462262));
    }

    {
        ((BarebonesFloridaDestinationFactory) handlinerDestinationStrategy)
            .setHabitIntercept(new FixedDoubleParameter(2.53163185));
        ((BarebonesFloridaDestinationFactory) handlinerDestinationStrategy)
            .setDistanceInKm(new FixedDoubleParameter(-0.00759009));
...

    {
        ...
        ((FloridaLogitReturnFactory) handlinerFishingStrategy)
            .setPriceRedGrouper(new FixedDoubleParameter(0.92395 / 2.20462262));
        ((FloridaLogitReturnFactory) handlinerFishingStrategy)
            .setPriceGagGrouper(new FixedDoubleParameter(-0.65122 / 2.20462262));
        ((FloridaLogitReturnFactory) handlinerFishingStrategy)
            .setRatioCatchToFishHold(new FixedDoubleParameter(4.37828));
    ...
    {
        {
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy)
                .setIntercept(new FixedDoubleParameter(-2.959116));
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy)
                .setSpring(new FixedDoubleParameter(0.770212));
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy)
                .setSummer(new FixedDoubleParameter(0.933939));
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy)
                .setWinter(new FixedDoubleParameter(0.706415));
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy).setWeekend(new NullParameter());
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy)
                .setWindSpeedInKnots(new FixedDoubleParameter(0.004265));
            ((FloridaLogisticDepartingFactory) longlinerDepartingStrategy)
                .setRealDieselPrice(new FixedDoubleParameter(-0.125913 / 0.219969157));
...

    {
        ...
        ((FloridaLogitReturnFactory) longlinerFishingStrategy)
            .setPriceRedGrouper(new FixedDoubleParameter(-0.12476 / 2.20462262));
        ((FloridaLogitReturnFactory) longlinerFishingStrategy)
            .setPriceGagGrouper(new FixedDoubleParameter(-0.28232 / 2.20462262));
        ((FloridaLogitReturnFactory) longlinerFishingStrategy)
            .setRatioCatchToFishHold(new FixedDoubleParameter(4.53092));
    ...
    public static void register() {
        Scenarios.add(
            "OSMOSE WFS",
            "A pre-set OSMOSE scenario to simulate the west florida shelf",
            OsmoseWFSScenario::new
        );
        MapInitializers.add("OSMOSE Map", OsmoseMapInitializerFactory.class);
        MapInitializers.add("OSMOSE Bounded Map", OsmoseBoundedMapInitializerFactory.class);
        BiologyInitializers.add("Osmose Biology", OsmoseBiologyFactory.class);
    }
    ...
    public ScenarioEssentials start(FishState model) {
        ...
        try {
            ...
            NauticalMapFactory.initializeMap(map, model.getRandom(), biology, weather, global, model);
                ...
                */
                ...
                for (Species species : global.getSpecies()) {
                    if (species.getName().trim().equals("RedGrouper"))
                        marketMap.addMarket(species, new FixedPriceMarket(redGrouperPrice.apply(model.getRandom())));
                    else if (species.getName().trim().equals("GagGrouper"))
                        marketMap.addMarket(species, new FixedPriceMarket(gagGrouperPrice.apply(model.getRandom())));
                    else if (species.getName().trim().equals("RedSnapper"))
                        marketMap.addMarket(species, new FixedPriceMarket(redSnapperPrice.apply(model.getRandom())));
                    else marketMap.addMarket(species, new FixedPriceMarket(-1d));
                }
                return marketMap;
            };
            ...
            TimeSeriesGasPriceMaker gasPriceMaker = new TimeSeriesGasPriceMaker(prices, true, IntervalPolicy.EVERY_DAY);
            longlinersPerPort = reader.readFile(
                mainDirectory.resolve("longline_ports.csv"), map, marketSupplier, gasPriceMaker, model);

            handlinersPerPort = reader.readFile(
                mainDirectory.resolve("handline_ports.csv"), map, marketSupplier, gasPriceMaker, model);

            for (Port port : reader.getPorts()) map.addPort(port);
            ...
            // start it!
            BiomassDrivenFixedExogenousCatches recreationalMortality =
                new BiomassDrivenFixedExogenousCatches(recast, false);
            model.registerStartable(recreationalMortality);

            // shrimp mortality
            LinkedHashMap<Species, Double> mortality = new LinkedHashMap<>();
            mortality.put(
                global.getSpecie("RedSnapper"), redSnapperMortalityFromShrimpBycatchInKg.apply(model.getRandom()));
            LinkedHashMap<Species, Pair<Integer, Integer>> bounds = new LinkedHashMap<>();
            bounds.put(global.getSpecie("RedSnapper"), new Pair<>(0, 1));

            OsmoseBoundedExogenousCatches shrimpMortality =
                new OsmoseBoundedExogenousCatches(mortality, bounds, "Shrimp-Bycatch Biomass Lost ");
    ...
    public ScenarioPopulation populateModel(FishState model) {
        ...
        Consumer<Fisher> predictorSetup = FishStateUtilities.predictorSetup(true, biology);
        ...
        final double mileage =
            2.82481; // this is litre per km and it comes from the california estimate of 1.21 gallons a mile
        ...
        for (Map.Entry<Port, Integer> entry : longlinersPerPort.entrySet())
            for (int i = 0; i < entry.getValue(); i++) {
                ...
                Fisher newFisher = new Fisher(
                    ...
                    new Boat(10, 10, new Engine(engineWeight, mileage, speed), new FuelTank(fuelCapacity)),
                    new Hold(longlinerHoldSize.apply(random), biology),
                    fisherGear,
                ...
                newFisher.addDockingListener((DockingListener) (fisher, port1) -> {
                    if (fisher.getHoursAtSea() > 0)
                        fisher.spendForTrip(longlinerTravellingCosts.apply(model.getRandom()) / fisher.getHoursAtSea());
                });
            }
        ...
        for (Map.Entry<Port, Integer> entry : handlinersPerPort.entrySet())
            for (int i = 0; i < entry.getValue(); i++) {
                ...
                Fisher newFisher = new Fisher(
                    ...
                    new Boat(10, 10, new Engine(engineWeight, mileage, speed), new FuelTank(fuelCapacity)),
                    new Hold(handlinerHoldSize.apply(random), biology),
                    fisherGear,
                    model.getSpecies().size()
                );
                ...
                newFisher.addDockingListener((DockingListener) (fisher, port1) -> {
                    if (fisher.getHoursAtSea() > 0)
                        fisher.spendForTrip(handlinerTravellingCosts.apply(model.getRandom()) / fisher.getHoursAtSea());
                });
            }
        ...
        networkBuilder.addPredicate(new NetworkPredicate() {
            @Override
            public boolean test(Fisher from, Fisher to) {
                if (from.getTags().contains("large")) return to.getTags().contains("large");
                else {
        ...
        return new ScenarioPopulation(fisherList, new SocialNetwork(networkBuilder), null);
    }
     ...
     */
    public void setLonglinerFishingStrategy(AlgorithmFactory<? extends FishingStrategy> longlinerFishingStrategy) {
     ...
     */
    public void setLonglinerGearStrategy(AlgorithmFactory<? extends GearStrategy> longlinerGearStrategy) {
    ...
    public void setLonglineLogbook(AlgorithmFactory<? extends LogbookInitializer> longlineLogbook) {
    ...
    public void setHandlineLogbook(AlgorithmFactory<? extends LogbookInitializer> handlineLogbook) {
    ...
    public void setHandlinerGear(AlgorithmFactory<? extends Gear> handlinerGear) {
    ...
    public void setHandlinerFishingStrategy(AlgorithmFactory<? extends FishingStrategy> handlinerFishingStrategy) {
    ...
    public void setHandlinerGearStrategy(AlgorithmFactory<? extends GearStrategy> handlinerGearStrategy) {
     ...
     */
    public void setRegulations(AlgorithmFactory<? extends Regulation> regulations) {
     ...
     */
    public void setRedSnapperMortalityFromShrimpBycatchInKg(DoubleParameter redSnapperMortalityFromShrimpBycatchInKg) {
